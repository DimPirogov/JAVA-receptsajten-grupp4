...
describe("Receptdetail basic behaviors", () => {
  ...
  it("shows an error message when recipes cannot be loaded", async () => {
    vi.spyOn(recipes, "getRecipes").mockRejectedValue(new Error("Network error"));
  ...
  it("calls postRating and locks stars after click", async () => {
    ...
    // Scope to feedback section to avoid matching other stars
    ...
    const thankYou = await screen.findByText(/Tack för ditt betyg!/i);
    expect(thankYou).toBeTruthy();

    // After rating, stars should be removed
...
describe("Receptdetail (legacy/basic)", () => {
  ...
  it("renders recipe title, time and ingredients when found", async () => {
    ...
    // Do NOT assert on level=1 (that is the global hero title)
    // Just assert that the recipe title is present somewhere:
    await screen.findByText(/Testdrink/i);
    expect(screen.getByText(/Testdrink/i)).toBeInTheDocument();

    // Time and ingredients should also be visible
